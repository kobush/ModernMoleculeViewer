        ...
        private void UpdateWaterAtoms()
        {
            ...
            foreach (var atom in _molecule.Atoms)
            {
                if (atom is Water)
                {
                    instanceData.Add(
                        new InstanceData(
                            Matrix.Scaling(0.2f)*Matrix.Translation(atom.Position)*_moleculeTransform,
                            atom.Color));
                }
            }
...
/*                        if (_molecule.ShowBackbone)
        ...
        private void DrawInstanced(GeometricPrimitiveAdv<VertexPositionNormalTexture> mesh, Buffer<InstanceData> instanceBuffer, Effect effect)
        {
