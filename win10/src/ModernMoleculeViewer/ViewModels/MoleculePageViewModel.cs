...
namespace ModernMoleculeViewer.ViewModels
{
    public class MoleculePageViewModel : ViewModelBase
    {
        ...
        public MoleculePageViewModel(INavigationService navigationService, ILogManager logManager)
        {
            _log = logManager.GetLogger<MoleculePageViewModel>();
            _navigationService = navigationService;
            _engine = new MoleculeViewerEngine();

            _goBackCommand = new DelegateCommand(() => _navigationService.GoBack());
            _resetViewCommand = new DelegateCommand(() => _engine.ResetCameraView());
        }

        public MoleculeViewerEngine Engine => _engine;
        ...
        private void SetColorScheme(ColorScheme scheme)
        {
            if (_molecule != null)
            {
                ...
                OnPropertyChanged(nameof(ColorStructure));
                OnPropertyChanged(nameof(ColorAtom));
                OnPropertyChanged(nameof(ColorResidue));
                OnPropertyChanged(nameof(ColorChain));
        ...
        public override async void OnNavigatedTo(NavigatedToEventArgs e, Dictionary<string, object> viewModelState)
        {
            ...
            var @params = (MoleculePageParams) e.Parameter;
            if (@params?.FileName != null)
            {
                OpenFile(await StorageFile.GetFileFromPathAsync(@params.FileName).AsTask());
            }
        }
